...
<html>
		...
		<script type="text/javascript">
			...
			function formCheck() {
				if($("#pw").val()!=<%=mDto.getPw()%>){
					alert("패스워드가 일치하지 않으면 수정할수 없습니다. 다시입력 ");
					$("#pw").focus();
					return false;
				}
	...
	<body>
      ...
      <form action="./modifyOk.jsp" id="form" name="form">
         ...
         <label>성별</label><br>
         <input type="radio" id="male" name="gender" value="male" <%=mDto.getGender().equals("male")?"checked":"" %> ><label for="male">남성</label>
         <input type="radio" id="female" name="gender" value="female" <%=mDto.getGender().equals("female")?"checked":"" %> ><label for="female">여성</label><br>
         <label>전화번호</label>
